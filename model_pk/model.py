...
class FeedForward(keras.layers.Layer):
    ...
    def __init__(self, input_dim=1, output_dim=32, out=False, output_fromLog=False):
        # activation='relu',
        super().__init__()
...
class MLP(keras.Model):
    ...
    def __init__(self, num_embed_layer=2, phys_dimension=1, embed_dim=32):
        ...
        for l in range(num_embed_layer):
            if l == 0:
                # HIDDEN: expand from differential equation's physical dimension to embedded dimension
                lay = FeedForward(input_dim=phys_dimension, output_dim=embed_dim, out=False)
                self.dense_layers.append(lay)
            elif l == num_embed_layer - 1:
                # OUTPUT: collapse from embedded dimension to physical dimension (of differential equation's physical dimension)
                lay = FeedForward(input_dim=embed_dim, output_dim=phys_dimension, out=True)
                self.dense_layers.append(lay)
            else:
                # HIDDEN: continue progressing through model graph in embedded dimension
                lay = FeedForward(input_dim=embed_dim, output_dim=embed_dim, out=False)
                self.dense_layers.append(lay)
